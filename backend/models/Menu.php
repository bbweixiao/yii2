...
class Menu extends \common\modelsgii\Menu
{
    ...
    public static function getMenusAll($rule = 'index/index'){
        $menus = [];
        /* 获取一级栏目 pid=0 and hide=0 */
        $menus['main']  = static::find()->where(['pid'=>0, 'hide'=>0])->orderBy('sort ASC')->asArray()->all();
        $menus['child'] = []; //设置子节点
        ...
        foreach ($menus['main'] as $key => $item) {
            ...
            $menuList = static::find()
                ->where(['pid'=>$item['id'], 'hide'=>0,  'url'=>$second_menu])
                ->orderBy('sort ASC')->asArray()->all();
            $menus['main'][$key]['_child'] = ArrayHelper::list_to_tree($menuList, 'id', 'pid', 'operater', $item['id']);
