...
class Menu extends \yii\widgets\Menu
{
    ...
    protected function isItemActive($item)
    {
        if (isset($item['url']) && is_array($item['url']) && isset($item['url'][0])) {
            ...
            if (count($route)==2&&$route[0] != $path[0] && $route !== $this->noDefaultRoute && $route !== $this->noDefaultAction) {
                return false;
            }
            if (count($route)==3&&$route[1] != $path[1] && $route !== $this->noDefaultRoute && $route !== $this->noDefaultAction) {
                return false;
            }
            return true;
        }
